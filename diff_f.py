...
def diff_f_avg(delta_f):
    ...
    stat_edge_user = np.zeros(M)

    for i in range(M):
        Task_compare[i] = sum(Task[i])
        stat_edge_user[i] = sum(a_final[i])
...
plt.legend(lns, labs, loc='upper right', prop=font_xy)
...
plt.tight_layout()
# plt.savefig("D:/python_code/access_v3/results/Fig10_compare_f.pdf")
