...
Task = [x1_Task / 30, x2_Task / 30, x3_Task / 30, x4_Task/30]
Var = [x1_Task_var/30, x2_Task_var/30, x3_Task_var/30, x4_Task_var/30]
...
plt.axhline(y=x1_Task / 30, color='blue', linestyle='--', linewidth=1)
ax1.set_ylabel('Average value of Task', font1, labelpad=10, color='blue')
plt.yticks(np.arange(2.4, 3.3, 0.15), color='blue', family='Times New Roman', fontsize=15)
plt.ylim(2.4, 3.15)
# 创建第二个坐标轴
ax2 = ax1.twinx()
# 绘制第二组数据（y2）
...
# plt.savefig("D:/python_code/access_v3/results/Fig6_bar_compare_Task.pdf")
plt.show()
